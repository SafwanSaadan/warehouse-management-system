  ...
  if(isset($_POST['update'])){
    ...
    $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"تم التحديث بنجاح ");
            ...
            <div class="col-md-8">
              ...
              <div class="form-group">
                <input type="hidden" name="user_id" value="<?php echo $user['id'];?>">
                 <button type="submit" name="submit" class="btn btn-warning">تغير</button>
              </div>
             </form>
            </div>
          </div>
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            ...
            <div class="form-group clearfix">
                    <a href="change_password.php" title="change password" class="btn btn-danger pull-right">تغير كلمة السر</a>
                    <button type="submit" name="update" class="btn btn-info">اضافة حساب</button>
            </div>
        </form>
      </div>
    </div>
