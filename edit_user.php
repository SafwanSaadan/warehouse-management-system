       ...
       $status   = remove_junk($db->escape($_POST['status']));
         ...
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"تم تحديث الملف ");
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          } else {
            $session->msg('d',' فشل التحديث');
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          }
    } else {
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
           <?php echo remove_junk(ucwords($e_user['name'])); ?> حساب
        </strong>
       </div>
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
            ...
            <div class="form-group">
              <label for="level">صفة المستخدم</label>
            ...
            <div class="form-group">
              <label for="status">الحالة</label>
                <select class="form-control" name="status">
                  <option <?php if($e_user['status'] === '1') echo 'selected="selected"';?>value="1">نشيط</option>
                  <option <?php if($e_user['status'] === '0') echo 'selected="selected"';?> value="0">غير نشيط</option>
                </select>
            </div>
            <div class="form-group clearfix">
                    <button type="submit" name="update" class="btn btn-info">تحديث</button>
            </div>
        </form>
       </div>
     </div>
  ...
  <div class="col-md-6">
    <div class="panel panel-default">
      <div class="panel-heading">
        <strong>
          <span class="glyphicon glyphicon-th"></span>
         <?php echo remove_junk(ucwords($e_user['name'])); ?>  تغير كلمة السر 
        </strong>
      </div>
      <div class="panel-body">
        <form action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" method="post" class="clearfix">
          ...
          <div class="form-group clearfix">
                  <button type="submit" name="update-pass" class="btn btn-danger pull-right">تغير</button>
          </div>
        </form>
      </div>
    </div>
